...
class QAgent(Agent):
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.epsilon = exp(-1)           # epsilon (p-wo akcji losowej)
        self.eps_min = exp(-5)           # końcowa wartość epsilon, poniżej którego już nie jest zmniejszane
    ...
    def choose_action(self, state: State) -> Action:
        ...
        if r > self.epsilon:
            ...
            return Action(self.action_space[best_action_idx])
        else:
            random_action_idx = np.random.choice(len(self.action_space))
            return  Action(self.action_space[random_action_idx])
